...
3. **Backend deploy (Tier 2).** In ThriveRadioPortal: `npm run db:push` (migrations 0062–0065), set `WHOOP_CLIENT_ID`/`WHOOP_CLIENT_SECRET` in Supabase function secrets, `supabase functions deploy whoop-link whoop-sync whoop-unlink`.
4. **~35 WHOOP API field/endpoint confirmations.** Marked "confirm against developer.whoop.com" in `_shared/whoop.ts` and `whoop-sync/index.ts` (token path, v1-vs-v2 base, pagination keys, recovery/sleep/cycle/workout field names). The `raw` jsonb column hedges schema drift.
